...
// Match Supabase's default 1000-row limit so pagination works correctly
const PAGE_SIZE = 1000;
const MAX_ROWS_PER_TABLE = 25000;
/** ga4_daily_summary session-level aggregates */
export const GA4_SUMMARY_MAX_ROWS = 120000;
const MAX_RETRIES = 3;
...
/**
 * @param {{ exactCount?: boolean }} [opts] — `count=exact` forces an extra DB count and often causes statement timeouts on large tables; use false for heavy paginated fetches (e.g. ga4_classified_pages).
 */
async function getAuthHeaders(opts = {}) {
  const useExactCount = opts.exactCount !== false;
  ...
  if (useExactCount) headers['Prefer'] = 'count=exact';
  return headers;
}

async function rawFetch(url, fetchOpts = {}) {
  const headers = await getAuthHeaders(fetchOpts);
...
/**
 * @param {{ exactCount?: boolean }} [opts] — Set `exactCount: false` for large tables to avoid expensive COUNT (statement timeouts).
 */
export async function sbFetchAllWithLimit(endpoint, maxRows = MAX_ROWS_PER_TABLE, opts = {}) {
  const sep = endpoint.includes('?') ? '&' : '?';
  const baseUrl = SUPABASE_URL + '/rest/v1/' + endpoint;
  const fetchOpts = { exactCount: opts.exactCount };

  const firstResult = await fetchWithRetry(baseUrl + sep + 'limit=' + PAGE_SIZE + '&offset=0', fetchOpts);
  ...
  // Return early only if we have all data
  if (totalCount != null && totalCount <= firstData.length) {
    return firstData;
  }
  ...
  if (totalCount != null && totalCount > firstData.length) {
    ...
    for (let batch = 1; batch < pages; batch += BATCH) {
      ...
      for (let p = batch; p < Math.min(batch + BATCH, pages); p++) {
        promises.push(
          fetchWithRetry(baseUrl + sep + 'limit=' + PAGE_SIZE + '&offset=' + (p * PAGE_SIZE), fetchOpts),
        );
      }
    ...
    if (all.length < totalCount) {
      console.log(`[Supabase] ${endpoint.split('?')[0]}: fetched ${all.length}/${totalCount} rows (capped at ${maxRows})`);
    }
    return all;
  }
  ...
  while (all.length < maxRows) {
    const { data } = await fetchWithRetry(baseUrl + sep + 'limit=' + PAGE_SIZE + '&offset=' + offset, fetchOpts);
...
export async function sbFetchAll(endpoint) {
  return sbFetchAllWithLimit(endpoint, MAX_ROWS_PER_TABLE);
}
...
/**
 * PostgREST query for ga4_daily_summary (session-accurate metrics).
 * @param {{ agencyId?: string|null, customerIds?: string[], dateFrom?: string, dateTo?: string, extra?: string }} opts
 */
export function buildGa4SummaryQuery({ agencyId, customerIds, dateFrom, dateTo, extra = '' } = {}) {
  let q = 'ga4_daily_summary?select=*';
  if (agencyId) q += `&agency_id=eq.${encodeURIComponent(agencyId)}`;
  if (customerIds && customerIds.length > 0) {
    const quoted = customerIds.map((id) => `"${String(id).replace(/"/g, '')}"`).join(',');
    q += `&customer_id=in.(${quoted})`;
  }
  if (dateFrom) q += `&report_date=gte.${dateFrom}`;
  if (dateTo) q += `&report_date=lte.${dateTo}`;
  q += extra;
  return q;
}

...
export function AuthProvider({ children }) {
  ...
  const hasPermission = useCallback((key) => {
    ...
    if (['super_admin', 'admin', 'manager'].includes(userRole?.toLowerCase())) return true;
  ...
  const loadUserProfile = useCallback(async (userId, authUser = null) => {
    ...
    try {
      ...
      /** Agency-wide data access: built-in roles OR explicit permission (custom roles often use the latter). */
      const viewAllByRole = isSuperAdmin || ['super_admin', 'admin', 'manager'].includes(roleName?.toLowerCase());
      const viewAllByPermission = permSet.has('customer.view_all')
        || permSet.has('action.manage_users')
        || permSet.has('action.admin_clients');
      const viewAll = viewAllByRole || viewAllByPermission;
      setCanViewAllCustomers(viewAll);

      if (!isSuperAdmin && !profile.agency_id) {
      ...
      if (!isSuperAdmin && viewAll && (cpaFiltered || []).length === 0) {
        console.warn(
          '[Auth] viewAll user has zero active client_platform_accounts for agency_id:',
          profile.agency_id,
          'Check user_profiles.agency_id and client_platform_accounts.agency_id/platform values.'
        );
      }

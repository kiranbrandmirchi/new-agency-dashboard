...
const ALL_TABS = [
  { id: 'campaigns', label: 'Campaigns', permission: 'tab.tiktok_ads.campaigns' },
  { id: 'adgroups', label: 'Ad Groups', permission: 'tab.tiktok_ads.adgroups' },
  { id: 'placements', label: 'Placements', permission: 'tab.tiktok_ads.placements' },
  { id: 'daily', label: 'Daily Breakdown', permission: 'tab.tiktok_ads.daily' },
];
...
export function TikTokPage() {
  ...
  const { filters, updateFilter, batchUpdateFilters, fetchData, loading, loadingPhase, error, kpis, campaigns, adGroups, placements, dailyTrend, tiktokAccounts } = useTikTokData();
  ...
  const [sort, setSort] = useState({ campaigns: { col: 'spend', dir: 'desc' }, adgroups: { col: 'spend', dir: 'desc' }, placements: { col: 'spend', dir: 'desc' }, daily: { col: 'report_date', dir: 'desc' } });
  const [pg, setPg] = useState({ campaigns: 1, adgroups: 1, placements: 1, daily: 1 });
  ...
  const handleApply = () => { setPg({ campaigns: 1, adgroups: 1, placements: 1, daily: 1 }); fetchData(); };
  ...
  useEffect(() => {
    ...
    const datasets = [
      { label: 'Spend', data: dailyTrend.map((d) => +(d.spend || d.cost || 0)), borderColor: '#FE2C55', backgroundColor: '#FE2C5518', tension: 0.35, fill: false, borderWidth: 2.5, yAxisID: 'y' },
      { label: 'Impressions', data: dailyTrend.map((d) => +(d.impressions || 0)), borderColor: '#25F4EE', backgroundColor: '#25F4EE18', tension: 0.35, fill: false, borderWidth: 2.5, yAxisID: 'y' },
      { label: 'Clicks', data: dailyTrend.map((d) => +(d.clicks || 0)), borderColor: '#000000', backgroundColor: '#00000012', tension: 0.35, fill: false, borderWidth: 2.5, yAxisID: 'y' },
    ];
  ...
  const colMap = { campaigns: campaignCols, adgroups: adGroupCols, placements: placementCols, daily: dailyCols };
  const dataMap = { campaigns, adgroups: adGroups, placements, daily: dailyTrend };
  ...
  const handleCSV = () => {
    ...
    exportCSV(cols.map((c) => ({ label: c.label, value: (r) => r[c.col] })), data, `tiktok-${activeTab}.csv`);
  };
  ...
  return (
    <div className="page-section active" id="page-tiktok">
      <div className="page-content">
        <div className="page-title-bar" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
          <div>
            <h2 style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <span style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: 32, height: 32, background: '#000', color: '#25F4EE', borderRadius: 8, fontSize: 14, fontWeight: 800 }}>♪</span>
              TikTok Ads
            </h2>
            <p>Campaign performance across TikTok Ads</p>
          </div>
        ...
        <div className="gads-filter-bar" id="gads-filter-bar-tiktok">
        ...
        <div className="gads-kpi-section">
          <div className="kpi-grid" id="gads-kpi-grid-tiktok">
            {KPI_ITEMS.map((item) => (
        ...
        <div id="gads-tab-content-tiktok">

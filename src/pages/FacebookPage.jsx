...
const ALL_TABS = [
  { id: 'campaigns', label: 'Campaigns', permission: 'tab.meta_ads.campaigns' },
  { id: 'adsets', label: 'Ad Sets', permission: 'tab.meta_ads.adsets' },
  { id: 'ads', label: 'Ads', permission: 'tab.meta_ads.ads' },
  { id: 'platforms', label: 'By Platform', permission: 'tab.meta_ads.platforms' },
  { id: 'placements', label: 'Placements', permission: 'tab.meta_ads.placements' },
  { id: 'daily', label: 'Daily Breakdown', permission: 'tab.meta_ads.daily' },
];
...
export function FacebookPage() {
  ...
  const { filters, updateFilter, batchUpdateFilters, fetchData, loading, loadingPhase, error, kpis, campaigns, adSets, ads, platforms, placements, dailyTrend, facebookAccounts } = useFacebookData();
  const TABS = React.useMemo(() => ALL_TABS.filter((t) => hasPermission(t.permission)), [hasPermission]);
  const defaultTab = TABS[0]?.id || 'campaigns';
  const [activeTab, setActiveTab] = useState(defaultTab);
  useEffect(() => {
    if (TABS.length && !TABS.some((t) => t.id === activeTab)) setActiveTab(defaultTab);
  }, [TABS, activeTab, defaultTab]);
  const [sort, setSort] = useState({ campaigns: { col: 'spend', dir: 'desc' }, adsets: { col: 'spend', dir: 'desc' }, ads: { col: 'spend', dir: 'desc' }, platforms: { col: 'spend', dir: 'desc' }, placements: { col: 'spend', dir: 'desc' }, daily: { col: 'report_date', dir: 'desc' } });
  const [pg, setPg] = useState({ campaigns: 1, adsets: 1, ads: 1, platforms: 1, placements: 1, daily: 1 });
  ...
  const handleApply = () => { setPg({ campaigns: 1, adsets: 1, ads: 1, platforms: 1, placements: 1, daily: 1 }); fetchData(); };
  ...
  useEffect(() => {
    ...
    const datasets = [
      { label: 'Spend', data: dailyTrend.map((d) => +(d.spend || 0)), borderColor: '#1877F2', backgroundColor: '#1877F218', tension: 0.35, fill: false, borderWidth: 2.5, yAxisID: 'y' },
  ...
  const campaignCols = [
    ...
    { col: 'purchase_count', label: 'Purchases', align: 'r', cell: (r) => fI(r.purchase_count || 0), value: (r) => r.purchase_count || 0 },
    { col: 'purchase_value', label: 'Purchase Value', align: 'r', cell: (r) => fU(r.purchase_value), value: (r) => r.purchase_value },
    { col: 'roas', label: 'ROAS', align: 'r', cell: (r) => fR(r.roas), value: (r) => r.roas },
    { col: 'cpa', label: 'CPA', align: 'r', cell: (r) => fU(r.cpa), value: (r) => r.cpa },
  ];

  const adSetCols = [
    { col: 'adset_name', label: 'Ad Set', cell: (r) => r.adset_name, value: (r) => r.adset_name },
    { col: 'impressions', label: 'Impressions', align: 'r', cell: (r) => fI(r.impressions), value: (r) => r.impressions },
    { col: 'clicks', label: 'Clicks', align: 'r', cell: (r) => fI(r.clicks), value: (r) => r.clicks },
    { col: 'ctr', label: 'CTR%', align: 'r', cell: (r) => fP(r.ctr), value: (r) => r.ctr },
    { col: 'spend', label: 'Spend', align: 'r', cell: (r) => fU(r.spend), value: (r) => r.spend },
  ...
  const adCols = [
    { col: 'ad_name', label: 'Ad', cell: (r) => r.ad_name, value: (r) => r.ad_name },
    { col: 'impressions', label: 'Impressions', align: 'r', cell: (r) => fI(r.impressions), value: (r) => r.impressions },
    { col: 'clicks', label: 'Clicks', align: 'r', cell: (r) => fI(r.clicks), value: (r) => r.clicks },
    { col: 'ctr', label: 'CTR%', align: 'r', cell: (r) => fP(r.ctr), value: (r) => r.ctr },
    { col: 'spend', label: 'Spend', align: 'r', cell: (r) => fU(r.spend), value: (r) => r.spend },
  ...
  const platformCols = [
    { col: 'platform', label: 'Platform', cell: (r) => r.platform, value: (r) => r.platform },
    { col: 'impressions', label: 'Impressions', align: 'r', cell: (r) => fI(r.impressions), value: (r) => r.impressions },
    { col: 'clicks', label: 'Clicks', align: 'r', cell: (r) => fI(r.clicks), value: (r) => r.clicks },
    { col: 'ctr', label: 'CTR%', align: 'r', cell: (r) => fP(r.ctr), value: (r) => r.ctr },
    { col: 'spend', label: 'Spend', align: 'r', cell: (r) => fU(r.spend), value: (r) => r.spend },
  ...
  const colMap = { campaigns: campaignCols, adsets: adSetCols, ads: adCols, platforms: platformCols, placements: placementCols, daily: dailyCols };
  const dataMap = { campaigns, adsets: adSets, ads, platforms, placements, daily: dailyTrend };
  ...
  const handleCSV = () => {
    ...
    exportCSV(cols.map((c) => ({ label: c.label, value: (r) => r[c.col] })), data, `facebook-${activeTab}.csv`);
  };
  ...
  return (
    <div className="page-section active" id="page-facebook">
      <div className="page-content">
        <div className="page-title-bar" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
          <div>
            <h2 style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <span style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: 32, height: 32, background: '#1877F2', color: 'white', borderRadius: 8, fontSize: 16, fontWeight: 700 }}>f</span>
              Meta / Facebook Ads
            </h2>
            <p>Campaign performance across Meta (Facebook & Instagram) Ads</p>
          </div>
        ...
        <div id="gads-tab-content">
          ...
          {!loading && activeTab === 'adsets' && renderTable('adsets', adSets, adSetCols)}
          {!loading && activeTab === 'ads' && renderTable('ads', ads, adCols)}
          {!loading && activeTab === 'platforms' && renderTable('platforms', platforms, platformCols)}

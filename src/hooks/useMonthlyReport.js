    ...
    try {
      ...
      for (const acc of accountsData || []) {
        ...
        if (platform === 'google_ads') {
          ...
          /** bing_campaign_daily contains both campaign-grain (ad_group_id='') and ad-group-grain rows; restrict to campaign-grain. */
          const [campRes, prevRes] = await Promise.all([
            supabase.from('bing_campaign_daily').select('*').eq('customer_id', cid).eq('ad_group_id', '').gte('report_date', currentFrom).lte('report_date', currentTo),
            supabase.from('bing_campaign_daily').select('*').eq('customer_id', cid).eq('ad_group_id', '').gte('report_date', prevFrom).lte('report_date', prevTo),
          ]);

          const campaignMap = new Map();
          (campRes.data || []).forEach((r) => {
            const key = r.campaign_id;
            if (!campaignMap.has(key)) campaignMap.set(key, { campaign_name: r.campaign_name, cost: 0, clicks: 0, impressions: 0, conversions: 0, purchase_value: 0 });
            const a = campaignMap.get(key);
            a.cost += num(r.spend); a.clicks += num(r.clicks); a.impressions += num(r.impressions);
            a.conversions += num(r.conversions || 0); a.purchase_value += num(r.conversions_value || 0);
          });
          const campaigns = [...campaignMap.values()].map((c) => ({
            ...c, cpc: c.clicks ? c.cost / c.clicks : 0, ctr: c.impressions ? (c.clicks / c.impressions) * 100 : 0,
          })).sort((a, b) => b.cost - a.cost);
          ...
          let pCost = 0, pClicks = 0, pImpr = 0, pConv = 0;
          (prevRes.data || []).forEach((r) => { pCost += num(r.spend); pClicks += num(r.clicks); pImpr += num(r.impressions); pConv += num(r.conversions || 0); });
          ...
          dataByAccount[acc.id] = {
            accountId: acc.id, label, platform,
            campaigns, keywords: [], daily,
            kpis: { cost: curCost, clicks: curClicks, impressions: curImpr, conversions: curConv, purchase_value: curPurchVal, cpc: curClicks ? curCost / curClicks : 0, ctr: curImpr ? (curClicks / curImpr) * 100 : 0 },
            prevKpis: { cost: pCost, clicks: pClicks, impressions: pImpr, conversions: pConv },
            momChange: { cost: momChange(curCost, pCost), clicks: momChange(curClicks, pClicks), impressions: momChange(curImpr, pImpr), conversions: momChange(curConv, pConv) },
          };

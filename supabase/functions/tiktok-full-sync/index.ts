...
function pickTokenString(obj, snake, camel) {
...
/** Fields aligned with tiktok_campaign_daily / tiktok_placement_daily (no cpc/ctr columns; UI derives those). */ function pickMetrics(m) {
  const spend = num(m.spend);
  const impressions = Math.round(num(m.impressions));
  const clicks = Math.round(num(m.clicks));
  const reach = Math.round(num(m.reach));
  const purchaseClicks = Math.round(num(m.complete_payment) || num(m.conversion) || num(m.result) || num(m.app_install));
  const purchaseVal = num(m.complete_payment_value ?? m.value_per_complete_payment ?? m.total_purchase_value ?? m.shopping_value ?? 0);
  return {
    impressions,
    clicks,
    spend,
...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    if (!appId || !appSecret) {
      return jsonRes({
        error: "TIKTOK_APP_ID / TIKTOK_APP_SECRET not configured",
        log: L
      }, 500);
    }
    log(`=== TIKTOK FULL SYNC === ${customerId} | ${dateFrom} → ${dateTo}`);
    const cpaRes = await fetch(`${SB_URL}/rest/v1/client_platform_accounts?platform_customer_id=eq.${encodeURIComponent(customerId)}&platform=eq.tiktok&is_active=eq.true&select=agency_id`, {
      headers: {
        apikey: SB_KEY,
    ...
    if (!Array.isArray(cpaRows) || !cpaRows.length) {
      return jsonRes({
        error: "No active TikTok account found for customer_id",
        log: L
      }, 400);
    }
    const agencyId = cpaRows[0].agency_id;
    const credRes = await fetch(`${SB_URL}/rest/v1/agency_platform_credentials?agency_id=eq.${agencyId}&platform=eq.tiktok&is_active=eq.true&select=oauth_refresh_token,oauth_access_token,oauth_token_expires_at,id`, {
      headers: {
        apikey: SB_KEY,
        Authorization: `Bearer ${SB_KEY}`
      }
    ...
    if (!credRow?.id) {
      return jsonRes({
        error: "No TikTok credential (connect TikTok in Settings)",
        log: L
      }, 400);
    }
    const credRowId = credRow.id;
    let refreshToken = credRow.oauth_refresh_token || "";
    ...
    if (refreshToken) {
      ...
      if (!accessToken) return jsonRes({
        error: "No access_token from refresh",
        log: L
      }, 400);
      ...
      return jsonRes({
        error: "TikTok access token expired or missing. Reconnect TikTok in Settings (this app received access-only tokens from TikTok).",
        log: L
      }, 400);
    }
    ...
    async function fetchReportPages(params) {
      ...
      while(true){
        ...
        qs.set("advertiser_id", customerId);
    ...
    /** campaign_id is not a valid dimension alongside adgroup_id; map adgroup → campaign via Ad Group API. */ async function fetchAdgroupCampaignMap(advertiserId, token) {
      const map = {};
      let page = 1;
      const pageSize = 1000;
      while(true){
    ...
    for (const day of dates){
      ...
      try {
        ...
        log(`campaign_daily ERR: ${e.message}`);
      }
      try {
        ...
        log(`placement_daily skip: ${e.message}`);
      }

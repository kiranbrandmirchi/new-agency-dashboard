...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    if (!FB_APP_ID || !FB_APP_SECRET) {
      return jsonResponse({
        error: "FB_APP_ID and FB_APP_SECRET must be set for Meta OAuth."
      }, 500);
    }
    ...
    if (action === "exchange_code") {
      ...
      if (shortData.error || !shortData.access_token) {
        const msg = typeof shortData.error === "object" ? shortData.error.message : shortData.error?.message || JSON.stringify(shortData).slice(0, 400);
        return jsonResponse({
          error: msg || "Facebook code exchange failed"
        }, 400);
      }
      ...
      const { data: existingRows, error: selErr } = await sb.from("agency_platform_credentials").select("id").eq("agency_id", agencyId).eq("platform", "facebook").limit(1);
      if (selErr) {
        console.error("[fb-oauth-connect] select:", selErr);
        return jsonResponse({
          error: "Failed to look up credentials: " + selErr.message
        }, 500);
      }
      const tokenPayload = {
      ...
      if (existingId) {
        const { error: updErr } = await sb.from("agency_platform_credentials").update(tokenPayload).eq("id", existingId);
        if (updErr) {
          console.error("[fb-oauth-connect] update:", updErr);
          return jsonResponse({
            error: "Failed to save credentials: " + updErr.message
          }, 500);
        }
      } else {
        ...
        if (insErr) {
          ...
          return jsonResponse({
            error: "Failed to save credentials: " + insErr.message
          }, 500);
        }
      }
      return jsonResponse({
    ...
    if (action === "disconnect") {
      const { error: updateErr } = await sb.from("agency_platform_credentials").update({
        is_active: false
      }).eq("agency_id", agencyId).eq("platform", "facebook");
      if (updateErr) {
        return jsonResponse({

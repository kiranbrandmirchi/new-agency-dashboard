...
/** Naive single-tag extractor (good enough for the small set of values we read from SOAP responses). */
...
function getFaultMessage(xml: string): string | null {
...
// ---------------------------------------------------------------------------
// Microsoft Ads Reporting API v13: SOAP report definitions.
// One column per ReportColumn element. "TimePeriod" -> date in CSV ("GregorianDate" header).
// ---------------------------------------------------------------------------

type ReportSpec = {
  key: string;
  reportName: string;
  scopeXml: (accountId: string) => string;
  columns: string[];
  toRow: (csv: Record<string, string>, customerId: string) => Record<string, unknown> | null;
  table: string;
  conflict: string;
};
...
const REPORTS: ReportSpec[] = [
  {
    ...
    scopeXml: (id) =>
      `<Scope><AccountIds xmlns:a1="http://schemas.microsoft.com/2003/10/Serialization/Arrays"><a1:long>${
        xmlEscape(id)
      }</a1:long></AccountIds></Scope>`,
    columns: [
      "TimePeriod",
      "AccountId",
      "CampaignId",
  ...
  },
  {
    ...
    toRow: (r, customerId) => {
      ...
      const locationName = [r.City, r.State, r.Country].filter(Boolean).join(", ") || r.Country || "";
      return {
        customer_id: customerId,
        campaign_id: String(r.CampaignId),
        campaign_name: r.CampaignName || null,
...
function buildSubmitEnvelope(opts: {
  ...
  const colXml = opts.columns
    .map((c) => `<ReportColumn>${xmlEscape(c)}</ReportColumn>`)
    .join("");
...
async function soapCall(action: string, envelope: string): Promise<{ ok: boolean; xml: string; status: number }> {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!clientId || !clientSecret || !developerToken) {
      return jsonRes({ error: "BING_CLIENT_ID / BING_CLIENT_SECRET / BING_DEVELOPER_TOKEN not configured", log: L }, 500);
    }
    ...
    const cpaRes = await fetch(
      `${SB_URL}/rest/v1/client_platform_accounts?platform_customer_id=eq.${
        encodeURIComponent(customerId)
      }&platform=eq.bing&is_active=eq.true&select=agency_id`,
      { headers: { apikey: SB_KEY, Authorization: `Bearer ${SB_KEY}` } },
    );
    const cpaRows = await cpaRes.json();
    if (!Array.isArray(cpaRows) || !cpaRows.length) {
      return jsonRes({ error: "No active Bing account found for customer_id", log: L }, 400);
    }
    const agencyId = cpaRows[0].agency_id as string;

    // ---- Lookup credential ----
    const credRes = await fetch(
      `${SB_URL}/rest/v1/agency_platform_credentials?agency_id=eq.${agencyId}&platform=eq.bing&is_active=eq.true&select=oauth_refresh_token,oauth_access_token,oauth_token_expires_at,id`,
      { headers: { apikey: SB_KEY, Authorization: `Bearer ${SB_KEY}` } },
    );
    const credRows = await credRes.json();
    const credRow = Array.isArray(credRows) ? credRows[0] : null;
    if (!credRow?.id) {
      return jsonRes({ error: "No Bing credential (connect Bing in Settings)", log: L }, 400);
    }
    const credRowId = credRow.id as string;
    const refreshToken = (credRow.oauth_refresh_token as string) || "";
    const storedAccess = (credRow.oauth_access_token as string) || "";
    const expAt = credRow.oauth_token_expires_at as string | null;

    let accessToken: string = storedAccess;
    const expiringSoon = !expAt || new Date(expAt).getTime() <= Date.now() + 60_000;
    if (!storedAccess || expiringSoon) {
      if (!refreshToken) {
        return jsonRes({ error: "Bing access token expired and no refresh token; reconnect in Settings.", log: L }, 400);
      }
    ...
    for (const spec of REPORTS) {
      try {
        ...
        log(`Report ${spec.key} threw: ${(e as Error)?.message || e}`);
      }
    }
    ...
    return jsonRes({ success: true, total_rows: totalRows, breakdown, log: L });
  } catch (err) {
    console.error("bing-full-sync:", err);
    return jsonRes({ error: "Internal server error", detail: String((err as Error)?.message || err) }, 500);
  }
});
